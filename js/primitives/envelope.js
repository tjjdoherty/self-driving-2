class Envelope {
    constructor(skeleton, width, roundness = 0) {
        this.skeleton = skeleton; // this is called storing as an attribute, skeleton is the 2 spinal points of the polygon
        this.poly = this.#generatePolygon(width, roundness);
    }

    #generatePolygon(width, roundness) {
        const { p1, p2 } = this.skeleton; // this is the destructuring assignment - i don't have to say "this.skeleton.p1" or this.skeleton.p2 now, just p1 or p2 for shorthand
        ...
        const alpha_ccw = alpha - Math.PI / 2; // these are points that are offset by 90 degrees clockwise and counter-clockwise from the original point
